<template>
    <b-overlay :show="isBusy" rounded="sm">
        <b-card 
            :header="$t('topActiveTenant_card_title')"
            class="TenantConsumption mt-2 mr-2"
        >
           <b-row v-if="loadStatSuccess">
            <b-col cols="12">
              <apexchart type="bar" height="400" :options="tenantConsumptionOption" :series="tenantConsumptionData"></apexchart>
            </b-col>
           </b-row>
...
export default Vue.extend({
    ...
    async fetch() {
        ...
        this.isBusy = true;
        this.$axios.get<TenantSetupStatsRespItf>(this.$config.tenantBasicStat+'/topactive',config)
            .then((response) =>{
